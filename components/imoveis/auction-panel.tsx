...
import { placeBid } from "@/app/imoveis/leiloes/actions";
...
// Mesma estrutura e mesma disciplina do painel de leilão do Gran Bazar
// (components/gran-bazar/auction-panel.tsx) — só muda a paleta (imoveis-*
// em vez de bazar-*) e o server action importado (@/app/imoveis/leiloes/
// actions em vez de @/app/gran-bazar/leiloes/actions). Esse server action
// chama a MESMA função Postgres gran_bazar_place_bid() (SECURITY DEFINER +
// lock de linha) — apesar do nome, é genérica entre módulos, ver
// docs/IMOVEIS.md.
export default function AuctionPanel({ auction, sellerId, currentUserId, bids }: AuctionPanelProps) {
  ...
  return (
    <div className="bg-amber-50 border border-amber-200 rounded-lg p-6 mb-6">
      ...
      <p className="text-sm text-imoveis-600 mb-4">
        {jaTerminou
          ? auction.winner_id
      ...
      {podeLicitar && !ehVendedor && currentUserId && (
        <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-2 mb-2">
          <div className="flex-1">
            ...
            />
            <p className="text-xs text-imoveis-600 mt-1">Mínimo: {formatEuros(minimoValido)} €</p>
          </div>
          <button
            type="submit"
            disabled={sending}
      ...
      {aDecorrer && acabouLocalmente && (
        <p className="text-sm text-imoveis-600 bg-white border border-amber-200 rounded-lg p-3">
      ...
      {bids.length > 0 && (
        <details className="mt-4">
          <summary className="text-sm text-imoveis-700 cursor-pointer hover:text-imoveis-900">
            Histórico de lances ({bids.length})
          </summary>
          <ul className="mt-2 space-y-1 max-h-48 overflow-y-auto">
            {bids.map((bid) => (
              <li key={bid.id} className="text-sm text-imoveis-800 flex justify-between border-b border-amber-100 py-1">
                <span>{bid.bidder_username ?? "Licitador"}</span>
                <span className="font-semibold">{formatEuros(bid.amount)} €</span>
                <span className="text-xs text-imoveis-500">{formatDataHora(bid.created_at)}</span>
              </li>
            ))}
          </ul>
        </details>
