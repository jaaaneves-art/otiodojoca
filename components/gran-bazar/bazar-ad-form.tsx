...
import { BAZAR_AD_TYPES, getBazarAdType } from "@/lib/gran-bazar/ad-types";
...
// <input type="datetime-local"> não sabe nada de fusos-horários: o valor
// que mostra e que devolve é sempre "hora de relógio" nua, sem offset. Se
// convertêssemos a partir de uma string UTC com slicing (ex:
// isoString.slice(0, 16)) ou com os getters UTC do Date, o utilizador veria
// (e submeteria) a hora errada sempre que o fuso local não for UTC+0. A
// forma correta é sempre passar pelos getters *locais* do Date — tanto ao
// mostrar (ISO → local) como ao submeter (local → ISO), e isto só pode
// acontecer no browser, nunca no servidor (onde "hora local" não faz
// sentido).
...
export function BazarAdForm({
  ...
  const config = getBazarAdType(tipo);
  const mostra = (campo: string) => config.fields.includes(campo as any);

  // Um leilão já agendado (com data/hora marcadas) só pode ter os seus
  // parâmetros alterados enquanto ainda não começou — depois disso já há
  // (ou pode haver a qualquer momento) lances em curso, e mudar o preço
  // inicial ou as datas seria injusto para quem já licitou. A base de
  // dados também impõe isto via RLS (ver migration de leilões); isto aqui
  // é só a experiência de utilizador correspondente.
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-6 rounded-xl border border-bazar-200 space-y-4">
      <div>
        <label className="text-sm font-medium">O que queres fazer? *</label>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-1">
          {Object.values(BAZAR_AD_TYPES).map((type) => (
            <button
              key={type.id}
              type="button"
              ...
              className={`py-2 px-3 rounded-lg border font-medium text-sm transition ${
                tipo === type.id
                  ? "bg-bazar-500 border-bazar-500 text-white"
                  : "border-bazar-200 text-bazar-800 hover:bg-bazar-50"
      ...
      <div>
        ...
        <input
          ...
          placeholder="Ex: Bicicleta de montanha"
          required
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="Descreve o que estás a anunciar..."
          required
          className="w-full border rounded-lg p-2 mt-1"
        />
      </div>
      ...
      {mostra("seeking") && (
        <div>
          <label className="text-sm font-medium">O que aceitas em troca? *</label>
          <input
            name="seeking"
            defaultValue={inicial?.seeking ?? ""}
            placeholder="Ex: Ferramenta elétrica, ou a combinar"
            required
            className="w-full border rounded-lg p-2 mt-1"
          />
        </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="0.00"
                required
                className="w-full border rounded-lg p-2 mt-1 disabled:bg-bazar-100 disabled:text-bazar-500"
              />
            </div>
            <div>
              ...
              <input
                ...
                min="0.01"
                disabled={leilaoBloqueado}
                defaultValue={inicial?.auction_minimum_increment ?? "1.00"}
                placeholder="1.00"
                className="w-full border rounded-lg p-2 mt-1 disabled:bg-bazar-100 disabled:text-bazar-500"
              />
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full border rounded-lg p-2 mt-1 disabled:bg-bazar-100 disabled:text-bazar-500"
              />
              <p className="text-xs text-bazar-600 mt-1">Deixa em branco para começar já</p>
            </div>
            <div>
      ...
      <button
        type="submit"
        className="w-full bg-bazar-500 text-white font-medium py-3 px-4 rounded-lg hover:bg-bazar-600"
      >
        {submitLabel}
      </button>
    </form>
