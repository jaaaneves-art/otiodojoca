...
export function TicketSelector({ sessionId, types }: { sessionId: number; types: Availability[] }) {
  ...
  return <form action={reserveOrder} className="space-y-3 rounded-xl border p-4">
    ...
    {types.map(t => <label key={t.ticket_type_id} className="flex items-center justify-between gap-3 text-sm">
      <span>{t.name} · {money(t.price_cents)}<small className="block text-slate-600">{t.sales_open ? `${t.available} disponíveis` : 'Vendas encerradas'}</small></span>
    ...
    <p className="text-xs text-slate-600">Reserva de 15 minutos. É necessário iniciar sessão. A disponibilidade é confirmada ao reservar.</p>
