import { ticketLabels } from '@/lib/espectaculos/presentation';
...
export function RefundForm({ orderId, tickets, reviewWithoutTickets }: { orderId: string; tickets: { id: string; status: string }[]; reviewWithoutTickets: boolean }) {
  return <form action={requestRefund} className="space-y-4 rounded border p-4">
    ...
    {tickets.map((t, i) => <label className="block" key={t.id}><input type="checkbox" name="ticket_id" value={t.id} disabled={t.status !== 'valid'} /> Bilhete {i + 1} · {ticketLabels[t.status]}</label>)}
    ...
    <SubmitButton>Registar pedido de reembolso</SubmitButton>
  </form>;
}
