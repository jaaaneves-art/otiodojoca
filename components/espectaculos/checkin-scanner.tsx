...
const labels: Record<string, string> = { accepted: 'Entrada aceite.', already_used: 'Bilhete já utilizado. Não permitir nova entrada.', invalid: 'Bilhete inválido para esta sessão.', error: 'Não foi possível validar. Confirma a ligação e as permissões.' };
export function CheckinScanner({ sessionId }: { sessionId: number }) {
  ...
  async function validate(value: string) {
    ...
    try { const result = await checkIn(sessionId, value, attempt.id); setMessage(labels[result.result] ?? labels.error); if (result.result !== 'error') { retry.current = null; setToken(''); } }
  ...
  return <section className="space-y-4">
    ...
    </form><p role="status" className="rounded border p-4 text-lg font-semibold">{busy ? 'A validar…' : message || 'Pronto para ler um bilhete.'}</p>
  </section>;
}
