...
describe('local operation without provider', () => {
 it('expires stock and reports deferred notifications without external payment calls', async () => {
  ...
  expect(await maintainTicketing()).toMatchObject({ expiredItems: 2, notificationsDeferred: 3, payments: 0 });
  expect(mocks.rpc).toHaveBeenCalledWith('event_expire_reservations');
  expect(mocks.from).toHaveBeenCalledTimes(1); expect(mocks.email).not.toHaveBeenCalled();
 });
 it('fails visibly when expiration fails', async () => {
  mocks.rpc.mockResolvedValue({ error: new Error('fixture') });
  await expect(maintainTicketing()).rejects.toThrow('expirar reservas');
 });
 it('default notification transport defers without acknowledging delivery', async () => {
  expect(await deferredNotifications.deliver({ id: 'fixture', kind: 'tickets_issued', orderId: 'fixture', userId: 'fixture', email: 'buyer@example.invalid', eventName: 'Evento' })).toBe('deferred');
