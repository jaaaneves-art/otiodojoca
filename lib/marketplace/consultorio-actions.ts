...
export async function atualizarConsultorio(adId: number, data: ConsultorioFormData) {
  try {
    ...
    // location/details. Sem p_category_id (consultórios têm categoria
    // fixa) — a RPC usa COALESCE e não a apaga.
...
export async function apagarConsultorio(adId: number) {
  try {
    ...
    if (!sucessoDelete) {
      return { sucesso: false, erro: 'Não foi possível apagar (ou não tens permissão para este consultório)' };
    }
