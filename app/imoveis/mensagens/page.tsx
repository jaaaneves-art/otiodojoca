...
export default async function InboxImoveisPage() {
  ...
  // as do módulo Imóveis, primeiro filtramos os ids de anúncios deste módulo.
  const { data: imoveisAds } = await supabase
    .from("marketplace_ads")
    .select("id")
    .eq("module", "imoveis");

  const imoveisAdIds = (imoveisAds || []).map((a: any) => a.id);

  let conversationList: any[] = [];
  if (imoveisAdIds.length > 0) {
    ...
    <>
      ...
      <div className="min-h-screen bg-imoveis-50">
        <main className="max-w-4xl mx-auto p-6">
          <h1 className="text-3xl font-bold text-imoveis-900 mb-6">💬 Mensagens</h1>

          {enriched.length === 0 ? (
            <div className="text-center py-16 bg-white rounded-lg border border-imoveis-200">
              <p className="text-imoveis-700 text-lg mb-4">Ainda não tens conversas</p>
              <Link href="/imoveis">
            ...
            <div className="space-y-3">
              {enriched.map((conv: any) => {
                ...
                return (
                  <Link key={conv.id} href={`/imoveis/mensagens/${conv.id}`}>
                    <div className={`bg-white rounded-lg border border-imoveis-200 hover:shadow-md transition p-4 flex gap-4 items-center cursor-pointer ${hasUnread ? "border-l-4 border-l-imoveis-600" : ""}`}>
                      {photo ? (
                        <img src={photo} alt={conv.ad?.title} className="w-16 h-16 object-cover rounded-lg" />
                      ) : (
                        <div className="w-16 h-16 bg-imoveis-50 rounded-lg flex items-center justify-center text-2xl">🏠</div>
                      )}
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-1">
                          <p className="font-semibold text-imoveis-900 truncate">{conv.ad?.title || "Imóvel removido"}</p>
                          {hasUnread && (
                            <span className="ml-2 bg-imoveis-600 text-white text-xs px-2 py-0.5 rounded-full">{conv.unreadCount}</span>
                          )}
                        </div>
                        <p className="text-sm text-imoveis-600 mb-1">
                          {conv.isBuyer ? "Anunciante" : "Interessado"}: <span className="font-medium">{otherName}</span>
                        </p>
                        <p className={`text-sm truncate ${hasUnread ? "text-imoveis-900 font-medium" : "text-imoveis-500"}`}>{previewTruncated}</p>
                      </div>
                      <div className="text-xs text-imoveis-500 whitespace-nowrap">
