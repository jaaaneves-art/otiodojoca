...
import { SubmitButton } from '@/components/espectaculos/submit-button';
...
export default async function OrderPage({ params }: { params: Promise<{ orderId: string }> }) {
  ...
  return <main className="mx-auto max-w-3xl space-y-5 p-6"><Link href="/espectaculos/encomendas">← Encomendas</Link>
    ...
    <OrderSummary order={order} items={items ?? []} /><p role="status">{checkoutState(order, observedAt).message}</p>
    ...
    {order.status === 'reserved' && unexpired && order.total_cents === 0 && (qrConfigured() ? <form action={confirmFree}><input type="hidden" name="order_id" value={order.id} /><SubmitButton>Confirmar bilhetes gratuitos</SubmitButton></form> : <p>A emissão de bilhetes ainda não está disponível.</p>)}
    {checkoutState(order, observedAt).payable && order.total_cents > 0 && <Link className="inline-block rounded bg-rose-600 px-4 py-2 text-white" href={`/espectaculos/checkout/${order.id}`}>Continuar para pagamento</Link>}
