import { dateTime, ticketLabels } from '@/lib/espectaculos/presentation';
...
export default async function TicketPage({ params }: { params: Promise<{ ticketId: string }> }) {
  ...
  const { data: ticket } = await db.from('event_tickets').select('id,status,order_item_id,key_version,checked_in_at').eq('id', uuid(ticketId)).maybeSingle();
  if (!ticket) notFound();
  ...
  return <main className="mx-auto max-w-xl space-y-5 p-6 text-center"><Link className="print:hidden" href={`/espectaculos/encomendas/${order.id}`}>← Encomenda</Link><h1 className="text-2xl font-bold">{order.purchase_snapshot.event_name}</h1><p>{item.name} · Bilhete não nominativo</p>
    ...
    <p>{order.purchase_snapshot.place || "Local a confirmar"}</p><p className="font-bold">Estado: {ticketLabels[ticket.status] ?? "Indisponível"}</p>{ticket.checked_in_at && <p>Entrada registada: {dateTime(ticket.checked_in_at)}</p>}
