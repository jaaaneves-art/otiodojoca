...
import { markAsRead } from "@/app/viaturas/mensagens/actions";
import MessageForm from "@/components/viaturas/message-form";

export default async function ConversationViaturaPage({
  params,
}: {
  ...
  return (
    <div className="min-h-screen bg-viaturas-50 flex flex-col">
      <nav className="bg-white border-b border-viaturas-200 px-6 py-4 sticky top-0 z-10">
      ...
      <main className="flex-1 overflow-y-auto p-6">
        <div className="max-w-3xl mx-auto space-y-3">
          {messages && messages.length > 0 ? (
              ...
              return (
                <div key={msg.id} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-[75%] rounded-2xl px-4 py-2 ${
                    isMe ? "bg-viaturas-600 text-white rounded-br-sm" : "bg-white border border-viaturas-200 text-viaturas-900 rounded-bl-sm"
                  }`}>
                          ...
                          if (isImage) {
                            return (
                              <a key={att.id} href={att.storage_path} target="_blank" rel="noopener noreferrer">
                                <img src={att.storage_path} alt={att.file_name} className="max-w-[220px] max-h-[220px] object-cover rounded-lg border border-viaturas-200 hover:opacity-90 transition" />
                              </a>
                            );
                          }
                          return (
                            <a key={att.id} href={att.storage_path} target="_blank" rel="noopener noreferrer"
                              className={`flex items-center gap-2 rounded-lg px-3 py-2 border ${isMe ? "bg-viaturas-500 border-viaturas-400 hover:bg-viaturas-400" : "bg-viaturas-50 border-viaturas-200 hover:bg-viaturas-100"}`}>
                              <span className="text-xl">📄</span>
                              <span className={`text-sm truncate max-w-[160px] ${isMe ? "text-white" : "text-viaturas-800"}`}>{att.file_name}</span>
                            </a>
                          );
                        })}
                      </div>
                    )}
                    <p className={`text-xs mt-1 ${isMe ? "text-viaturas-100" : "text-viaturas-500"}`}>
            ...
            <div className="text-center py-8 text-viaturas-500">Sem mensagens ainda. Envia a primeira!</div>
          )}
        </div>
      </main>

      <div className="bg-white border-t border-viaturas-200 p-4 sticky bottom-0">
