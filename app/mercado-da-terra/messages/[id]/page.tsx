...
export default async function ConversationPage({
  ...
  // Confirmar que o utilizador atual faz parte desta conversa
  if (conv.buyer_id !== user.id && conv.seller_id !== user.id) {
    notFound();
  }

  // Conversa de um anúncio de outro módulo (Gran Bazar) — não pertence aqui.
  if (!conv.ad || (conv.ad as any).module !== "mercado-da-terra") {
    notFound();
  }
  ...
  return (
    <div className="min-h-screen bg-terra-50 flex flex-col">
      {/* Header (próprio da conversa, com o nome do interlocutor - mantido de propósito) */}
