 ...
 * (app/gran-bazar/mensagens/actions.ts) — mesma lógica, sobre as mesmas
 * tabelas marketplace_conversations/marketplace_messages (genéricas,
 * ligadas só por ad_id). Só existe como ficheiro à parte porque o
 * redirect e o revalidatePath têm de apontar para /lup/mensagens.
 */
export async function startConversation(formData: FormData) {
  ...
  redirect(`/lup/mensagens/${conversationId}`);
}
...
export async function sendMessage(formData: FormData) {
  ...
  revalidatePath(`/lup/mensagens/${conversationId}`);
  revalidatePath("/lup/mensagens");
}
...
export async function markAsRead(conversationId: number) {
  ...
  // render da página da conversa (app/lup/mensagens/[id]/page.tsx), e o
  ...
  // pedido já traz os dados frescos; a lista em /lup/mensagens já corre
