    ...
    <>
      ...
      <div className="min-h-screen bg-lup-50">
        <main className="max-w-4xl mx-auto p-6">
          <h1 className="text-3xl font-bold text-lup-900 mb-6">💬 Mensagens</h1>

          {enriched.length === 0 ? (
            <div className="text-center py-16 bg-white rounded-lg border border-lup-200">
              <p className="text-lup-700 text-lg mb-4">Ainda não tens conversas</p>
              <Link href="/lup">
              ...
              {enriched.map((conv: any) => {
                ...
                return (
                  <Link key={conv.id} href={`/lup/mensagens/${conv.id}`}>
                    <div className={`bg-white rounded-lg border border-lup-200 hover:shadow-md transition p-4 flex gap-4 items-center cursor-pointer ${hasUnread ? "border-l-4 border-l-lup-500" : ""}`}>
                      {photo ? (
                        <img src={photo} alt={conv.ad?.title} className="w-16 h-16 object-cover rounded-lg" />
                      ) : (
                        <div className="w-16 h-16 bg-lup-50 rounded-lg flex items-center justify-center text-2xl">♻️</div>
                      )}
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-1">
                          <p className="font-semibold text-lup-900 truncate">{conv.ad?.title || "Anúncio removido"}</p>
                          {hasUnread && (
                      ...
                      <div className="text-xs text-lup-500 whitespace-nowrap">
