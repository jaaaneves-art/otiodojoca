...
**Correção — 2026-08-27:** o item "decidir o destino do 'Lup Imóveis' (nome, se avança)" listado aqui inicialmente estava desatualizado — a decisão **já tinha sido tomada em 24/08**, antes desta sessão. `claude/AUDITORIA-LUP-IMOVEIS-20260824.md` auditou o MVP React + prompt externo "Lup Imóveis" e recomendou não reaproveitar o nome nem o esquema (colidia com o módulo "Lup" de economia circular já em produção, e propunha tabelas `properties`/`property_auctions` paralelas às já existentes). Foi construído como módulo **"Imóveis"** (sem prefixo "Lup"), reaproveitando `marketplace_ads`/`marketplace_photos`/o motor de leilões do Gran Bazar — ver `supabase/migrations/20260824010000_imoveis.sql` e `claude/RELATORIO-IMOVEIS-CONTINUACAO-20260824.md`. Migração aplicada, `npm run build` confirmado limpo, rotas `/imoveis/*` publicadas em `origin/main`. Não há decisão de nome pendente — só falta o teste manual, já coberto pela linha "Testar fluxos manuais de Lup e Imóveis" acima.
